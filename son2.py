...
def format_btc_whale_report(all_period_data, all_xchain_data,
    ...
    for i, (label, data) in enumerate(all_period_data):
        ...
        oran = (
            abs(fark_usd) / gunluk_hacim * 100
        ) if (gunluk_hacim and hacim_var) else 0
        oran_s = f"{oran:.2f}%"
        out.append(
            f"\n{label}\n"
...
def btc_kisavadeli_analizler(ohlcv_dict, current_price, dtstr_tr, dtstr_utc):
    ...
    for vade, ohlcv in vadeler:
        ...
        ema7 = ema7_arr[-1] if ema7_arr is not None else None
        ema21 = ema21_arr[-1] if ema21_arr is not None else None
        macd_tuple = macd(close, 12, 26, 9)
        macd_line = macd_tuple[0][-1] if macd_tuple[0] is not None else None
        rsi_arr = rsi(close, 14)
        rsi_val = rsi_arr[-1] if rsi_arr is not None else None
        mfi_arr = mfi(high, low, close, volume, 14)
        mfi_val = mfi_arr[-1] if mfi_arr is not None else None
        atr_arr = atr(high, low, close, 14)
        atr_val = atr_arr[-1] if atr_arr is not None else None
        ...
        # GÖSTERGE METİNLERİ
        ema_text = 'Pozitif' if trend == 'Pozitif' else 'Negatif' if trend == 'Negatif' else 'Veri yok'
        macd_text = 'Pozitif' if macd_line is not None and macd_line > 0 else 'Negatif' if macd_line is not None and macd_line <= 0 else 'Veri yok'
        rsi_text = f"RSI: {rsi_val:.2f}" if rsi_val is not None else "RSI: Veri yok"
        mfi_text = f"MFI: {mfi_val:.2f}" if mfi_val is not None else "MFI: Veri yok"
        atr_text = f"ATR: {atr_val:.2f}" if atr_val is not None else "ATR: Veri yok"

        results.append(
            f"📉 {vade} Analiz: {signal} {strength} | "
            f"EMA7/21: {ema_text} | "
            f"MACD: {macd_text} | "
            f"{rsi_text} | {mfi_text} | {atr_text} | Volatilite: {vol_txt}"
        )
        if vade == "5dk":
            results.append(
...
def btc_teknik_analiz_raporu(
    ...
    rapor.append(f"• RSI: {rsi_val:.2f}" if rsi_val is not None else "• RSI: Veri yok")
    rapor.append(f"• StochRSI: {stochrsi_val:.2f}" if stochrsi_val is not None else "• StochRSI: Veri yok")
    rapor.append(f"• MFI: {mfi_val:.2f}" if mfi_val is not None else "• MFI: Veri yok")
    rapor.append(f"• ADX: {adx_val:.2f} → {'Güçlü trend var.' if adx_val is not None and adx_val > 25 else 'Trend zayıf.' if adx_val is not None else 'Veri yok'}")
    rapor.append(f"• OBV: {obv_val:.2f} → {'Alış baskısı var.' if obv_val is not None and obv_val > 0 else 'Satış baskısı var.' if obv_val is not None and obv_val < 0 else 'Veri yok'}")
...
def plot_technical_indicators(ohlcv):
    ...
    ema21 = ema(closes, 21)
    if ema7 is not None:
        plt.plot(ema7[-100:], label='EMA7', linestyle='--')
    if ema21 is not None:
        plt.plot(ema21[-100:], label='EMA21', linestyle='--')
    plt.title('Fiyat ve EMA Trendi')
    plt.legend()
    plt.subplot(3, 1, 2)
    rsi_arr = rsi(closes, 14)
    if rsi_arr is not None:
        plt.plot(rsi_arr[-100:], label='RSI', color='purple')
    plt.axhline(70, linestyle='--', color='red')
    plt.axhline(30, linestyle='--', color='green')
    plt.title('RSI Göstergesi')
    plt.subplot(3, 1, 3)
    plt.bar(range(len(ohlcv['volume'][-100:])), ohlcv['volume'][-100:], color='gray')
...
def backtest_strategy(ohlcv_data, lookback=50, rsi_period=14, macd_fast=12, macd_slow=26, macd_signal=9, take_profit=0.01, stop_loss=0.01, hold_bars=5):
    ...
    for i in range(lookback, len(closes) - hold_bars):
        try:
            ...
            rsi_arr = rsi(current_slice['close'], rsi_period)
            macd_tuple = macd(current_slice['close'], macd_fast, macd_slow, macd_signal)
            if rsi_arr is None or macd_tuple[0] is None:
                continue
            rsi_val = rsi_arr[-1]
            macd_line = macd_tuple[0][-1]
        except Exception:
            continue
...
async def main_loop():
    async with TelegramClient('whalealert_session', api_id, api_hash) as client:
        await client.start()
        all_messages = []
